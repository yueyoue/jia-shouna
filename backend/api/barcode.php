...
switch ($action) {
    case 'lookup':
        ...
        $errors = [];
        foreach ($apis as $api) {
            $result = tryLookupBarcode($db, $user, $barcode, $api);
            if ($result['found']) {
                success($result);
            }
        ...
        success([
            'found' => false,
            'barcode' => $barcode,
            'msg' => '所有接口均未找到该条码: ' . implode('; ', $errors)
        ]);
        break;
 ...
 * 尝试用单个接口查询条码
 */
function tryLookupBarcode($db, $user, $barcode, $api) {
    $startTime = microtime(true);
    $apiUrl = $api['api_url'];

    // 构建请求URL - 支持 {barcode} 占位符
    ...
    if (!empty($api['api_key']) || !empty($api['api_secret'])) {
        if (strpos($url, 'mxnzp.com') !== false) {
            // RollToolsApi: 替换空的 app_id 和 app_secret
    ...
    try {
        ...
        // 根据不同接口解析返回数据，统一格式
        $parsed = normalizeBarcodeResponse($data, $api['name']);
        if ($parsed) {
...
/**
 * 统一解析不同接口的返回格式
 * 返回标准格式: ['name' => '', 'brand' => '', 'category' => '', 'spec' => ''] 或 null
 */
function normalizeBarcodeResponse($data, $apiName) {
    // RollToolsApi (mxnzp) 格式: {"code": 0, "data": {"goodsName": "...", "goodsBrand": "..."}}
    ...
    // Open Food Facts 格式: {"product": {"product_name": "...", "brands": "...", "categories": "..."}}
    ...
    // ApiZero 格式: {"data": {"goodsName": "...", "brand": "..."}}
